...
@section("content")
    ...
    <div class='alert alert-success'><b>Deleted that post</b>
        <br/><a href='{{ route('blog.admin.index') }}' class='btn btn-primary '>Back to posts overview</a></div>

    <?php
    $images_to_delete = [];
    foreach ((array) config("blog.image_sizes") as $image_size => $image_size_info) {
    ...
    @if(count($images_to_delete))
        ...
        <table class='table'>
            ...
            @foreach($images_to_delete as $image_size)
                ...
                <tr>
                    ...
                    <td class='text-center'><a
                                href='{{asset(config("blog.blog_upload_dir","blog_images")."/".$deletedPost->$image_size) }}'
                                target='_blank' class='btn btn-primary m-1'>view</a>

                        <img src='{{asset(config("blog.blog_upload_dir","blog_images")."/".$deletedPost->$image_size) }}'
                             width=100>
            ...
            {{--check filesize returns something, so we don't divide by 0--}}
                        @if(filesize(public_path(config("blog.blog_upload_dir","blog_images")."/".$deletedPost->$image_size)))

                        ({{ (round(filesize(public_path(config("blog.blog_upload_dir","blog_images")."/".$deletedPost->$image_size)) / 1000 ,1)). " kb"}})

                        @endif

                    </td>
                    <td><code>
                            <small>{{ public_path(config("blog.blog_upload_dir","blog_images")."/".$deletedPost->$image_size) }}</small>
                        </code></td>
                </tr>
