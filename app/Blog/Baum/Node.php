...
use App\Blog\Baum\Extensions\Eloquent\Collection;
use App\Blog\Baum\Extensions\Eloquent\Model;
...
abstract class Node extends Model
{
     ...
     * @param array $models
     *
     * @return \App\Blog\Baum\Extensions\Eloquent\Collection
     */
    ...
    {
        $withHighestRight = $this->newNestedSetQuery()->reOrderBy($this->getRightColumnName(), 'desc')->take(1)
            ->sharedLock()->first();
    ...
    public function setDepth()
    {
        ...
        $this->getConnection()->transaction(function () use ($self)
        {
            ...
            $self->newNestedSetQuery()->where($self->getKeyName(), '=', $self->getKey())
                ->update(array($self->getDepthColumnName() => $level));
    ...
    public function setDepthWithSubtree()
    {
        ...
        $this->getConnection()->transaction(function () use ($self)
        {
            ...
            $self->newNestedSetQuery()->where($self->getKeyName(), '=', $self->getKey())
                ->update(array($self->getDepthColumnName() => $newDepth));
     ...
     *
     * @param App\Blog\Baum\Node|int $target
