@php
    use Modules\Booking\Helpers\Booking;
@endphp
<x-theme::app-layout>
    <div class="w-full lg:w-2/3 mx-auto">
        ...
        <div class="flex flex-col lg:flex-row gap-6">
            ...
            <div class="w-full lg:w-3/5 space-y-4">
                @if ($classes->isEmpty())
                    <h3 class="text-xl font-semibold text-gray-900 mt-2">{{ __('Nothing scheduled yet')  }}</h3>
                    <span>{{ __('Schedule ahead and start speaking! Explore') }}
                        <a href="{{ route('booking::index') }}" target="_blank"
                           class="inline-flex items-center mt-4 text-xm font-semibold hover:underline text-blue-700"
                        >
                            {{ __('My course') }}
                        </a>
                    </span>
                            ...
                            <div class="flex flex-col lg:flex-row items-start lg:items-center gap-6">
                                ...
                                <div class="w-full">
                                    ...
                                    <div class="flex items-center text-sm text-gray-600 space-x-2 mt-1">
                                        ...
                                        <span>{{ $class->lesson_type->label() }} class</span>
                                    ...
                                    <div class="flex items-center text-sm text-gray-600 space-x-2 mt-1">
                                        ...
                                        <span>{{ Booking::formatDate($class->slot_start_at, 'H:i A') }} – {{ Booking::formatDate($class->slot_end_at, 'H:i A') }} {{ Booking::getUserGMT($class->student) }}</span>
                                    </div>
