<?php
/**
 * User: Denis Popov
 * Date: 20.10.2017
 * Time: 22:30
 */
return [
    'back' => 'Назад',
    'logout' => 'Выйти',
    'You have uploaded images successfully' => 'You have uploaded images successfully',
    'have.no.access.to.admin.area' => 'У вас не достаточно прав для доступа в админ часть',
    'to_admin' => 'Админ. секция',
    'Password' => 'Пароль',
    'Reset Password' => 'Восстановление пароля',
    'reset_password_submit' => 'Сбросить пароль',
    'E-Mail Address for reset' => 'Укажите свой емеил адрес',
    'Send Password Reset Link' => 'Отправить ссылку для восстановления',
    'E-Mail Address' => 'Емаил адрес',
];
