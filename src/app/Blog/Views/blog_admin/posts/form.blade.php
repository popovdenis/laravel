...
<div class='row'>
    ...
    <div class='col-sm-12 col-md-4'>
        ...
        <div class="form-group">
            ...
            <small id="blog_slug_help" class="form-text text-muted">The slug (leave blank to auto generate) - i.e.{{route("blog.single", [app('request')->get('locale'), "your-slug"])}}</small>
        </div>
...
@if(config("blog.use_custom_view_files",true))
    <div class="form-group">
...
@if(config("blog.image_upload_enabled",true))
    ...
    <div class='bg-white pt-4 px-4 pb-0 my-2 mb-4 rounded border'>
        ...
        @foreach(config("blog.image_sizes") as $size_key =>$size_info)
        ...
        @if($loop->iteration>1)
            ...
            @endif
                ...
                @if($post_translation->has_image($size_info['basic_key']))
                    <a style="color: darkred" href="{{route("blog.admin.remove_photo",[
...
@else
    <div class='alert alert-warning'>Image uploads were disabled in blog.php config</div>
@endif
...
<div class='bg-white pt-4 px-4 pb-0 my-2 mb-4 rounded border'>
    ...
    <div class='row'>
        ...
        <div class='col-md-12 my-3 text-center'>
            ...
            <em><a class="a-link-cart-color" target='_blank' href='{{route("blog.admin.categories.create_category")}}'><i class="fa fa-external-link" aria-hidden="true"></i>
                    Add new categories
                    here</a></em>
        </div>
    </div>
