...
class PostTranslation extends Model implements SearchResultInterface
{
    ...
    {
        if (array_key_exists("image_" . $size, config("blog.image_sizes"))) {
            return true;
        }
        ...
        if (starts_with($size, "image_")) {
            ...
            /* the config/blog.php and the DB columns SHOULD have keys that start with image_$size
            however when using methods such as image_url() or has_image() it SHOULD NOT start with 'image_'

            To put another way: :
                in the config/blog.php : config("blog.image_sizes.image_medium")
                in the database table:    : blog_posts.image_medium
                when calling image_url()  : image_url("medium")
            */
            throw new \InvalidArgumentException("Invalid image size ($size). Post image size should not begin with 'image_'. Remove this from the start of $size. It *should* be in the blog.image_sizes config though!");
        }

        throw new \InvalidArgumentException("Post image size should be 'large','medium','thumbnail' or another field as defined in config/blog.php. Provided size ($size) is not valid");
    }
