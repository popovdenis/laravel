...
class Search implements SearchInterface
{
     ...
     * @param string $search
     *
     * @return \Illuminate\Database\Eloquent\Collection|\App\Blog\FulltextSearch\IndexedRecord[]
     */
    ...
    public function searchQuery($search)
    {
        ...
        $titleWeight = str_replace(',', '.', (float) config('blog.search.weight.title', 1.5));
        $contentWeight = str_replace(',', '.', (float) config('blog.search.weight.content', 1.0));
        ...
        $query = IndexedRecord::query()
            ...
            ->limit(config('blog.search.limit-results'));

        $query->with([
            'indexable' => function ($query)
            {
                $query->with([
                    'post' => function ($query)
                    {
